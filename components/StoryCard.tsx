...
export default function StoryCard({
  ...
  return (
    ...
    >
      <article className="flex flex-col h-full bg-card text-card-foreground border border-border/50 shadow-md hover:shadow-2xl hover:shadow-accent/10 hover:border-accent/30 hover:-translate-y-2 transition-all duration-300 ease-out rounded-2xl overflow-hidden ring-1 ring-black/5 dark:ring-white/10 notranslate">
        ...
        <div className="flex-1 flex flex-col p-6">
          ...
          <div className="mt-auto pt-4 border-t border-border flex items-center justify-between text-xs font-bold uppercase tracking-wider text-muted-foreground">
            <div className="flex items-center gap-2 truncate pr-2">
              {/* Cleaned up metadata as per user request */}
            </div>
            {article.viewCount !== undefined && (
              <span className="shrink-0 mr-3 flex items-center gap-1">
                <svg
                  className="w-3 h-3"
                  fill="none"
                  stroke="currentColor"
